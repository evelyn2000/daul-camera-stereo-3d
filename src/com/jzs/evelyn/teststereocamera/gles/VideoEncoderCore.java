...
public class VideoEncoderCore {
    ...
    private static final int IFRAME_INTERVAL = 1;//5;           // 5 seconds between I-frames
...
//    	mRecordingFile = new File(outputDir, "VID_" + format.format(calendar.getTime()) + ".mp4");
    	if(true){
	        ...
	        // We're not actually interested in multiplexing audio.  We just want to convert
	        // the raw H.264 elementary stream we get from MediaCodec into a .mp4 file.
	    	mMuxer = new MediaMuxer(mRecordingFile, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
...
//        }
    ...
    public void drainEncoder(boolean endOfStream) {
        ...
        if (VERBOSE) Logger.d(TAG, "drainEncoder(" + endOfStream + ")");

        if (endOfStream) {
            if (VERBOSE) Logger.d(TAG, "sending EOS to encoder");
        ...
        while (true) {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                if (!endOfStream) {
                    ...
                    if (VERBOSE) Logger.d(TAG, "no output available, spinning to await EOS");
                }
                ...
                if ((mBufferInfo.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                    ...
                    if (VERBOSE) Logger.d(TAG, "ignoring BUFFER_FLAG_CODEC_CONFIG");
                    mBufferInfo.size = 0;
                }
                ...
                if ((mBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                    if (!endOfStream) {
                        ...
                        if (VERBOSE) Logger.d(TAG, "end of stream reached");
                        mMuxerStarted = false;
