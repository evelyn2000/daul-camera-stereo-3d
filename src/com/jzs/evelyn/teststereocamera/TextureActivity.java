...
public class TextureActivity extends Activity {
	private final static String TAG = "Jzs.TextureActivity";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		surface = (GLSurfaceView) findViewById(R.id.surface_view);
	...
	private class TextureRenderer implements GLSurfaceView.Renderer {
		...
		// These two methods help to Load/Unload the shaders used by OpenGL Es
		// 2.0
		// Remember that now OpenGL DOES NOT CONTAIN most of the 'old' OpenGL
		// functions;
		...
		public void setup() {
			...
			// Now things start to get interesting. Take note of a new
			// attribute,
			// aTexPos, that will store the texture coordinate (the "places" of
			// the texture that
			// we will use. We also have vTexPos, to pass the attribute value to
			// the
			// fragment shader.
		...
		public void tearDown() {
			if (programHandle != -1) {
				...
				GLES20.glDeleteTextures(textures.length, textures, 0); // free
																		// the
																		// texture!
			}
		}

		// auxiliary shader functions. Doesn't matter WHAT you're trying to do,
		// they're
		...
		public void onSurfaceCreated(GL10 gl, EGLConfig config) {
			...
			Bitmap b = BitmapFactory.decodeStream(getResources().openRawResource(R.raw.sample_sbs));
...
//					R.drawable.skull);
		...
		public void onSurfaceChanged(GL10 gl, int width, int height) {
			...
			int uScreenPos = GLES20.glGetUniformLocation(programHandle,
					"uScreen");
			int uTexture = GLES20.glGetUniformLocation(programHandle,
					"uTexture");
			// The uScreen matrix
			...
			FloatBuffer b = ByteBuffer.allocateDirect(uScreen.length * 4)
					.order(ByteOrder.nativeOrder()).asFloatBuffer();
			b.put(uScreen).position(0);
			GLES20.glUniformMatrix4fv(uScreenPos, b.limit() / uScreen.length,
					false, b);
			// Activate the first texture (GL_TEXTURE0) and bind it to our
			// handle
			...
			// since we're using a PNG file with transparency, enable alpha
			// blending.
		...
		public void onDrawFrame(GL10 gl) {
			...
			int aPosition = GLES20.glGetAttribLocation(programHandle,
					"aPosition");
			int aTexPos = GLES20.glGetAttribLocation(programHandle, "aTexPos");
			// Ok, now is the FUN part.
			// First of all, our image is a rectangle right? but in OpenGL, we
			// can only draw
			// triangles! To remedy that we will use 4 vertices (V1 to V4) and
			// draw using
			// the TRIANGLE_STRIP option. If you look closely to our positions,
			// you will note
			// that we're drawing a 'N' (or 'Z') shaped line... and
			// TRIANGLE_STRIP 'closes' the
			// remaining GAP between the vertices, so we have a rectangle (or
			// square)! Yay!
			//
			// Apart from V1 to V4, we also specify the position IN THE TEXTURE.
			// Each vertex
			// of our rectangle must relate to a position in the texture. The
			// texture coordinates
			// are ALWAYS 0,0 on bottom-left and 1,1 on top-right. Take a look
			// at the values
			// used and you will understand it easily. If not, mess a little bit
			// with the values
			// and take a look at the result.
			float[] data = { 50f, 100f, // V1
			...
			FloatBuffer b = ByteBuffer.allocateDirect(data.length * FLOAT_SIZE)
					.order(ByteOrder.nativeOrder()).asFloatBuffer();
			b.put(data);
			// Position of our image
			b.position(POSITION_OFFSET);
			GLES20.glVertexAttribPointer(aPosition, POSITION_SIZE,
					GLES20.GL_FLOAT, false, TOTAL_SIZE * FLOAT_SIZE, b);
			GLES20.glEnableVertexAttribArray(aPosition);
			// Positions of the texture
			b.position(TEXTURE_OFFSET);
			GLES20.glVertexAttribPointer(aTexPos, TEXTURE_SIZE,
					GLES20.GL_FLOAT, false, TOTAL_SIZE * FLOAT_SIZE, b);
