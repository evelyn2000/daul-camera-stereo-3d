...
public class Drawable2d {
     ...
     */
    public Drawable2d(Prefab shape, float[] coord, float[] tex_coord) {
        switch (shape) {
            case TRIANGLE:
            	if(coord == null || tex_coord == null){
	                mVertexArray = TRIANGLE_BUF;
	                mTexCoordArray = TRIANGLE_TEX_BUF;
            	} else {
            		mVertexArray = GlUtil.createFloatBuffer(coord);
	                mTexCoordArray = GlUtil.createFloatBuffer(tex_coord);
            	}
                mCoordsPerVertex = 2;
    ...
    public boolean isTransparentRound(){
    	return false;
    }
